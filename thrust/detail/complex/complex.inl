...
template <typename T>
THRUST_HOST_DEVICE complex<T>::complex()
{
  real(T());
  imag(T());
}
...
#if THRUST_CPP_DIALECT < 2011
template <typename T>
THRUST_HOST_DEVICE complex<T>::complex(const complex<T>& z)
{
  real(z.real());
  imag(z.imag());
}
...
#if THRUST_CPP_DIALECT < 2011
template <typename T>
THRUST_HOST_DEVICE complex<T>& complex<T>::operator=(const complex<T>& z)
{
  real(z.real());
  imag(z.imag());
  return *this;
